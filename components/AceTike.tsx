    ...
    if (year >= 2004) {
      ...
      data.forEach((point: any) => {
        if (point['34kt_wind_nm'] && point['50kt_wind_nm'] && point['64kt_wind_nm']) {
          ...
          // Convert nautical miles to meters (mean radius)
          const r34 =
            ((wind34.ne + wind34.se + wind34.sw + wind34.nw) / 4) * 1852;
          const r50 =
            ((wind50.ne + wind50.se + wind50.sw + wind50.nw) / 4) * 1852;
          const r64 =
            ((wind64.ne + wind64.se + wind64.sw + wind64.nw) / 4) * 1852;

          // Ensure proper ordering (safety against bad data)
          const R34 = Math.max(r34, r50, r64);
          const R50 = Math.max(Math.min(r34, r50), r64);
          const R64 = Math.min(r34, r50, r64);

          // Define annulus areas (NO overlap)
          const area64 = Math.PI * Math.pow(R64, 2);
          const area50 = Math.PI * (Math.pow(R50, 2) - Math.pow(R64, 2));
          const area34 = Math.PI * (Math.pow(R34, 2) - Math.pow(R50, 2));
